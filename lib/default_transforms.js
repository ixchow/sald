...
function loadAudioGeneric(mimeType, encoding) {
	...
	return function loadAudio(file) {
		...
		var script = [
			'var CustomAudio = require("sald:CustomAudio.js");',
			'var loadedFile = "' + loadFileGeneric(file, encoding) + '";',
			//'var loadedFile = loadFile("' + file + '", "' + encoding + '");',
			'module.exports = new CustomAudio("' + mimeType + '", "' + encoding + '", loadedFile);'
		].join('\n');
		return script;
