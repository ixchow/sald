...
function processDependencies( blobs, cname, jstxt ){
	...
	for (var i = 0; i < dependencies.length; i ++)
	{
		...
		{
			var errorString = "Could not find a transform for dependency type: " + deptype;

			// Temporary fix for audio support
			// TODO: convert supported audio files to ogg
			if (deptype == '.mp3' || deptype == '.wav' 
				|| deptype == '.aif' || deptype == '.mid' 
				|| deptype == '.midi'){

				errorString += ", use .ogg file type for audio files instead."
			}
